function rt(textData)
    if textData.army and textData.army ~= GetFocusArmy() then
        return true
    end
    return false
end
...
function PrintToScreen(textData)
    if not locations[textData.location] then
        ...
        control:SetColor(color) control:SetFont(UIUtil.bodyFont, textData.size)
        control.active = true control:Show() control:SetAlpha(1)
        if textData.duration then
            control.time = 0 control:SetNeedsFrameUpdate(true)
            control.OnFrame = function(self, time)
                ...
                if self.time > textData.duration then
                    ...
                    if newAlpha < 0 then
                        self:Hide()
                        self.active = false self:SetNeedsFrameUpdate(false)
                    else
